...
import fetchLookupData from '@salesforce/apex/Ex_GREUpdateDetails.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/Ex_GREUpdateDetails.fetchDefaultRecord';
const DELAY = 300; // dealy apex callout timing in miliseconds  

export default class Ex_CustomlookupResuable extends LightningElement {
     // public properties with initial default values 
     @api label = 'Channel Partner';
    ...
    // initial function to populate default selected lookup record if defaultRecordId provided  
     ...
     @wire(fetchLookupData, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName' })
      searchResult(value) {
        console.log('fetch cp2');
         const { data, error } = value; // destructure the provisioned value
         this.isSearchLoading = false;
         if (data) {
            console.log('fetch cp2 data');
              this.hasRecords = data.length == 0 ? false : true; 
              this.lstResult = JSON.parse(JSON.stringify(data)); 
              console.log('hasRecords'+JSON.stringify(this.hasRecords));
              console.log('1stresutl'+JSON.stringify(this.lstResult));
          }
         else if (error) {
