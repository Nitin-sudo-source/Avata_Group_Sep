...
import getOppDetails from '@salesforce/apex/Ex_CreateQuotationController.getOppDetails';
import getUnitDetails from '@salesforce/apex/Ex_CreateQuotationController.getUnitDetails';
import getPaymentSchemeDetails from '@salesforce/apex/Ex_CreateQuotationController.getPaymentSchemeDetails';
import getCarParkDetails from '@salesforce/apex/Ex_CreateQuotationController.getCarParkDetails';
import getPriceListMapDetails from '@salesforce/apex/Ex_CreateQuotationController.getPriceListMapDetails';
import getPriceListGroupMapDetails from '@salesforce/apex/Ex_CreateQuotationController.getPriceListGroupMapDetails';
import getDiscountGroupMapDetails from '@salesforce/apex/Ex_CreateQuotationController.getDiscountGroupMapDetails';
import getAllPriceMapDetails from '@salesforce/apex/Ex_CreateQuotationController.getAllPriceMapDetails';
import getAllPriceInfoFormattedMap from '@salesforce/apex/Ex_CreateQuotationController.getAllPriceInfoFormattedMap';
import getPaymentScheduleDetails from '@salesforce/apex/Ex_CreateQuotationController.getPaymentScheduleDetails';
import getPicklistValues from '@salesforce/apex/Ex_CreateQuotationController.getPicklistValues';
import validateUpdatedPaymentScheduleDetails from '@salesforce/apex/Ex_CreateQuotationController.validateUpdatedPaymentScheduleDetails';
import getUpdatedPaymentScheduleDetails from '@salesforce/apex/Ex_CreateQuotationController.getUpdatedPaymentScheduleDetails';
import getCalculatedNPVDetails from '@salesforce/apex/Ex_CreateQuotationController.getCalculatedNPVDetails';
import getModifiedPaymentScheduleDetails from '@salesforce/apex/Ex_CreateQuotationController.getModifiedPaymentScheduleDetails';
import saveQuotationDetails from '@salesforce/apex/Ex_CreateQuotationController.saveQuotationDetails';

// import getUnitDetails from '@salesforce/apex/Ex_CreateQuotationController.getUnitDetails';
// import getOppDetails from '@salesforce/apex/Ex_CreateQuotationController.getOppDetails';
// import getPaymentSchemeDetails from '@salesforce/apex/Ex_CreateQuotationController.getPaymentSchemeDetails';
// import getCarParkDetails from '@salesforce/apex/Ex_CreateQuotationController.getCarParkDetails';
// import getDiscountDetails from '@salesforce/apex/Ex_CreateQuotationController.getDiscountDetails';
// import getAllPricingInfo from '@salesforce/apex/Ex_CreateQuotationController.getAllPricingInfo';
// import getPaymentSummaryDetails from '@salesforce/apex/Ex_CreateQuotationController.getPaymentSummaryDetails';
// import save from '@salesforce/apex/Ex_CreateQuotationController.save';
// import getOppFromProject from '@salesforce/apex/Ex_CreateQuotationController.getOppFromProject';
// import PaymentSchemeEditableDetails from '@salesforce/apex/Ex_CreateQuotationController.PaymentSchemeEditableDetails';
// import discountVisible from '@salesforce/apex/Ex_CreateQuotationController.discountVisible'; //carParkVisible
// import carParkVisible from '@salesforce/apex/Ex_CreateQuotationController.carParkVisible'; 

export default class Ex_CreateQuate extends LightningElement {
    @api uId;
    @api oppId;
    @track unit;
    @track opp = [];
    ...
    @track npvCalculatedMap = [];
    @track rrValue;
    @track rrValueStr = '';
    @track allIncusiveAmount = 0;
    @track modifiedAVRate = 0;
        ...
        getUnitDetails({ uId: this.uId })
            .then(data => {
                if (data) {
                    ...
                    this.unit = data;
                    //this.rrValue = this.unit.RR_Amount__c;
                    /*if (this.rrValue != '' && this.rrValue != null) {
                        this.rrValueStr = this.rrValue.toLocaleString('en-IN', {
                            style: 'decimal',
                            maximumFractionDigits: 2,
                          });    
                    }*/

                    if (this.unit.Sale_Status__c !== 'Vacant') { //Sale_Status__c
                        this.isError = true;
            ...
            .then(data => {
                if (data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        console.log('data[i].Name: ' + JSON.stringify(data[i].Name));
                ...
                console.log(JSON.stringify('paymentSchemeList:' + JSON.stringify(this.paymentSchemeList)));
                //console.log('this.paymentMilestoneWrapperList: '+JSON.stringify(this.paymentMilestoneWrapperList));

            })
    }

    getCarPark() {
        getCarParkDetails({ pId: this.unit.Tower__r.Project__c, tId: this.unit.Tower__c, unit: this.unit })
            .then(data => {
                if (data) {
    ...
    getPriceListGroupMap() {
        getPriceListGroupMapDetails({ uId: this.unit.Id })
            .then(data => {
                if (data) {
                    //console.log('priceListGroupMap: '+JSON.stringify(data));
                    /*var conts = data;
                    for(var key in data) {
                        this.priceListGroupMap.push({value:conts[key], key:key});
                    }*/
    ...
    // getDiscountGroupMap() {
    //     getDiscountGroupMapDetails({ pId: this.unit.Tower__r.Project__c, tId: this.unit.Tower__c, unit: this.unit })
    //         .then(data => {
    //             if (data) {
        ...
        .then(data => {
           // alert('Test2');
            if (data != null) {
                ...
                console.log('OUTPUT : ',JSON.stringify(this.originalDiscountGroupMap));
                this.updatedDiscountGroupMap = JSON.parse(JSON.stringify(data));
                console.log('OUTPUT updatedDiscountGroupMap: ',JSON.stringify(this.updatedDiscountGroupMap));
                ...
                for (let group in this.updatedDiscountGroupMap) {
                    ...
                    const updatedDiscountList = [];
                    console.log('discountList:Inloop '+JSON.stringify(discountList));
                    ...
                    this.discountGroupMap.push({ key: group, value: updatedDiscountList });
                    console.log('finalDiscountGroupMap: '+JSON.stringify(this.discountGroupMap));
                }
            // } else if (error) {
    ...
    handleChange(event) {
        ...
        if (eventName === 'carParkRequiredCount') {
            ...
            if (value < 0) {
                ...
                this.showErrorMessage('Warning', 'Lumpsum Amount Cannot be Increased Beyond What is Already Defined'+ this.originalDiscountGroupMap[key][index].Amount__c);
                event.target.value = this.updatedDiscountGroupMap[key][index].Amount__c;
                return;
            } else {
            ...
            if(value < 0) {
                ...
                const psfAmount = this.originalDiscountGroupMap[key][index].PSF_Amount__c;
                console.log('2: '+psfAmount);
                this.showErrorMessage('Warning', 'PSF Cannot be Increased Beyond What is Already Defined : ' + psfAmount);
                event.target.value = this.updatedDiscountGroupMap[key][index].PSF_Amount__c;
                return;
            } else {
            ...
            if(value < 0) {
                ...
                this.showErrorMessage('Warning', 'Percentage Cannot be Increased Beyond What is Already Defined ' + this.originalDiscountGroupMap[key][index].Percentage_of_AV__c);
                event.target.value = this.updatedDiscountGroupMap[key][index].Percentage_of_AV__c;
                return;
            } else {
                const newObj = { ...this.updatedDiscountGroupMap[key][index], [eventName]: value };
            ...
            if (value) {
                ...
                if (groupIndex > -1) { // only splice array when item is found
            ...
            this.isSpinner = false;
        } else if (eventName === 'No_of_Days__c') {
            var value = parseInt(event.target.value);
            console.log('value: ' + value);
            ...
            console.log('No_of_Days__c 1: ' + this.updatedPaymentMilestoneWrapperList[index].pm.No_of_Days__c);

            const pmObj = { ...this.paymentMilestoneWrapperList[index].pm, [eventName]: value };
            this.paymentMilestoneWrapperList = [...this.paymentMilestoneWrapperList];
            this.paymentMilestoneWrapperList[index].pm = pmObj;
            console.log('No_of_Days__c 2: ' + this.paymentMilestoneWrapperList[index].pm.No_of_Days__c);
            this.isSpinner = false;
        ...
        getAllPriceMapDetails({ uId: this.unit.Id, priceListGroupMap: this.priceListGroupMap, priceListMap: this.priceListMap, carParkAmount: this.totalCarParkAmount, discountAmount: this.totalDiscountAmount })
            .then(data => {
                if (data) {
                    ...
                    // Calculate the total inclusive amount
                    let agreementValue = parseFloat(this.allPriceInfoMap['Agreement Value']) || 0;
                    let agreementValueTax = parseFloat(this.allPriceInfoMap['Agreement Value TAX']) || 0;
                    let otherCharges = parseFloat(this.allPriceInfoMap['Other Charges']) || 0;
                    let otherChargesTax = parseFloat(this.allPriceInfoMap['Other Charges TAX']) || 0;
                    let statutoryCharges = parseFloat(this.allPriceInfoMap['Statutory Charges']) || 0;
                    let statutoryChargesTax = parseFloat(this.allPriceInfoMap['Statutory Charges TAX']) || 0;
                    let saleableArea = parseFloat(this.unit.Saleable_Area__c) || 0;

                    let totalInclusiveAmount = agreementValue +
                        agreementValueTax +
                        otherCharges +
                        otherChargesTax +
                        statutoryCharges +
                        statutoryChargesTax;
                    let totalAVRateAmount = agreementValue / saleableArea;
                    

                    // Format the total inclusive amount
                    this.allIncusiveAmount = totalInclusiveAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                    this.modifiedAVRate = totalAVRateAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    updatePaymentSchedule() {
        ...
        for (let i = 0; i < this.updatedPaymentMilestoneWrapperList.length; i++) {
            if (!this.updatedPaymentMilestoneWrapperList[i].isTotal) {
                ...
                if (this.updatedPaymentMilestoneWrapperList[i].pm.No_of_Days__c === null) {
                    errorCount++;
    ...
    getCalculatedNPV() {
        getCalculatedNPVDetails({ isPaymentScheduleUpdated: this.isPaymentScheduleUpdated, paymentMilestoneWrapperList: this.updatedPaymentMilestoneWrapperList, u: this.unit })
            .then(data => {
                if (data) {
                    console.log('data: ' + JSON.stringify(data));
                    this.npvCalculatedMap.push(data);
                    for (let key in data) {
                        if (key === 'Original NPV') {
                        ...
                        //this.npvCalculatedMap.push({key: key, value: data[key]});
                    }
                    console.log('npvCalculatedMap: ' + JSON.stringify(this.npvCalculatedMap));
                    this.isSpinner = false;
                } else if (error) {
    ...
    saveQuotation() {
        this.isSpinner = true;
        console.log('npvCalculatedMap: ' + JSON.stringify(this.npvCalculatedMap));
        saveQuotationDetails({ u: this.unit, oppId: this.opp.Id, selectedSchemeId: this.selectedPaymentScheme, priceListGroupMap: this.priceListGroupMap, allPriceInfoMap: this.allPriceInfoMap, appliedDiscountList: this.appliedDiscountList, carParkList: this.updatedCarParkList, paymentMilestoneWrapperList: this.updatedPaymentMilestoneWrapperList, npvCalculatedMap: this.npvCalculatedMap })
            .then(data => {
                if (data) {
                    console.log('data: ' + JSON.stringify(data));
                    this.isSpinner = false;
                    if (data !== null) {
                        location.replace('/' + data);//apex/SignaturePad?id=
                    } else {
