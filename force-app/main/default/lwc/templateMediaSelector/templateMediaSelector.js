...
import getStaticResources from '@salesforce/apex/Ex_TemplateMediaSelectorHandler.getStaticResources';
import checkPublicURL from '@salesforce/apex/Ex_TemplateMediaSelectorHandler.checkPublicURL';
import Submit from '@salesforce/apex/Ex_TemplateMediaSelectorHandler.UpdateTemplate';
import searchTemplateRecord from '@salesforce/apex/Ex_TemplateMediaSelectorHandler.searchTemplateRecord';
import LightningConfirm from 'lightning/confirm';


export default class TemplateMediaSelector extends LightningElement {
    ...
    @track searchTemplateResult = [];
    @track showTemplateResult = false;
    @track indexkey ;
    ...
    StaticResource(){
        ...
        .then((result) => {
            ...
            const modifiedResult = result.map(record => {
                ...
                return { ...record, Extension: extension, template__c: '',showTemplateError :false,showTemplateResult : false };
            });
    ...
    handleTemplate(event){
        ...
        if(name === 'Whatsapp_Template_Name__c'){
            this.searchTemplate = fieldValue;
            if (fieldValue == '') {
                this.searchTemplateResult = [];
                this.showTemplateResult = false;
                this.showTemplateError = true; 

                const new_addobj = { ...this.StaticResourceList[this.indexkey], 'showTemplateResult': false, 'showTemplateError': true}
    ...
    searchTemplateRecords(){
        searchTemplateRecord({searchTerm: this.searchTemplate})
        .then((result) => {
            this.searchTemplateResult = result;
            this.showTemplateResult = true;

            const new_addobj = { ...this.StaticResourceList[this.indexkey], 'showTemplateResult': true, 'showTemplateError': false}
            console.log('new_addobj = '+ JSON.stringify(new_addobj));

            this.StaticResourceList[this.indexkey] = new_addobj;
            console.log('StaticResourceList: ' + JSON.stringify(this.StaticResourceList));
            console.log('Search Result .: ', JSON.stringify(this.searchTemplateResult));
        }).catch(error => {
    ...
    handlechange(evt){
        alert('');
        const TemplateId = evt.currentTarget.dataset.id;
        ...
        const new_addobj = { ...this.StaticResourceList[key], 'template__c': name, 'showTemplateResult': false, 'Id': TemplateId}
        console.log('new_addobj = '+ JSON.stringify(new_addobj));

        this.StaticResourceList[key] = new_addobj;
        console.log('StaticResourceList: ' + JSON.stringify(this.StaticResourceList));

        checkPublicURL({tempId: TemplateId})
        .then((result) => {
            console.log(JSON.stringify(result));
            if(result){
                this.execute = this.handleConfirmClick(key,TemplateId);
    ...
    async handleConfirmClick(key,TemplateId) {
        ...
        if(result){
            this.StaticResourceList[key].template__c = TemplateId;
            console.log('StaticResourceList = ' + JSON.stringify(this.StaticResourceList));
        }else{
            this.StaticResourceList[key].template__c = null;
            console.log('StaticResourceList = ' + JSON.stringify(this.StaticResourceList));
        }
    }
    ...
    handleSubmit(){
        ...
        for (const key in this.StaticResourceList) {
            ...
            console.log(this.StaticResourceList[key].template__c);

            if (this.StaticResourceList[key].template__c != null && this.StaticResourceList[key].template__c != undefined && this.StaticResourceList[key].template__c != '') {
                
                const element = {
                    'Id': this.StaticResourceList[key].Id,
                    'Public_Link_1__c': 'https://customization-velocity-7183--satyamsb.sandbox.my.salesforce-sites.com/ResourceLink/resource/'+ this.StaticResourceList[key].Name
                };
        ...
        Submit({WhatsappTemplate: this.templateList})
        .then((result) => {
            //console.log(result);
